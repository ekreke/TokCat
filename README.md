...
- 弹窗内可**多轮追问**，显示**完整对话记录**（你的提问 + agent 回答，用户/回答分列气泡；附件以缩略图展示）；仅保留当前会话的内存历史，切换本地/远程、改目录或菜单「断开 Agent」会重连并清空。自动跟随最新回答，向上翻阅时不会被强制拉到底部（附「回到底部」按钮）。
...
顶栏仅 **趋势图 / 设置 ▸ / 退出 TokCat** 三项，全部可调项收进「设置」子菜单：

- **趋势图**：顶部内嵌最近 5 分钟消耗折线，每个客户端一条，底部图例显示小计，顶部一行统计**最近 1 秒 / 合计 / 单秒峰值**（逐秒指标单位为 `t/s`，合计为 `t`）。
- **设置 ▸**：
  - **Hermes**：本地 / 远程(SSH) 切换，「Hermes 设置…」，「断开 Agent」。
...
- **左键**打开状态窗口，而非 Chat。
- 右键菜单：状态行 +「设置 ▸」（速率口径 / 尺寸 / 最大帧率 / 灵敏度 / 数据源 / 重载定价 / 重置统计 / 开机自启）+「退出 TokCat」。
- 暂无 Chat 与趋势图。

...
open class Highlighter {
    ...
    */
    public init?() {
        ...
        // 资源已并入 TokCatApp 的 SPM 资源包（见 AppBundle.swift），不再使用
        // Bundle.module：其生成的 accessor 在分发形态下必然找不到 bundle。
        let bundle = Bundle.tokCatResources

        // Load the highlight.js code from the bundle or fail
        guard let highlightPath: String = bundle.path(forResource: "highlight.min", ofType: "js", inDirectory: "Highlighter") else {
            return nil
        }
    ...
    */
    ...
    public func setTheme(_ themeName: String, withFont: String? = nil, ofSize: CGFloat? = nil) -> Bool {
        ...
        guard let themePath = self.bundle.path(forResource: themeName, ofType: "css", inDirectory: "Highlighter/styles") else {
            return false
        }
    ...
    */
    public func availableThemes() -> [String] {
        ...
        let paths = bundle.paths(forResourcesOfType: "css", inDirectory: "Highlighter/styles") as [NSString]
        var result = [String]()
        for path in paths {
